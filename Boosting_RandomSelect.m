...
function dt = getdecisionstump(X, Y, W)
    ...
    j = randi(col,1);
    uniX  = unique(X(:,j));
    i = randi(size(uniX,1),1);
    threshold = uniX(i,1);
    ll = find(X(:,j) > threshold);
    rl = find(X(:,j) <= threshold);
    llerr = geterror(Y,ll,W);
    rlerr = geterror(Y,rl,W);        
    error = llerr.error + rlerr.error;
    

    feature_index = j;
    
    class = zeros(2,1);
    class(1,1) = llerr.label;
    class(2,1) = rlerr.label;
