...
public class Spam {
	...
	public void StartProcess(){
		...
		ArrayList<String[]> trainingall = new ArrayList<String[]>(a);
		
		int T = 50;
		
		Double[][] result = new Double[T][test.size()];
		...
		for(int t=0; t<T; t++){
			...
			Collections.shuffle(trainingall);
			
			ArrayList<String[]> training = new ArrayList<String[]>();
			for(int n =0; n<1000; n++){
				training.add(trainingall.get(n));
			}
			...
			//ArrayList<TestRs> rateT = newtest.start(training);
			
				
			//System.out.println("Training Error Rate"+" "+rateT);			
			
			ArrayList<TestRs> rate = newtest.start(test);
			//System.out.println("Testing Error Rate"+" "+rate);
			
			
			int prenum = 0;
			for(TestRs trs:rate){
				result[t][prenum] = trs.pre;
				
				labels[prenum] = trs.real;
				prenum ++;
			}
			
			//ArrayList<TestRs> mse = newtest.start2(test);
		...
		System.out.println("Start calculate mean");
		double[] sum = new double[test.size()];
		double[] mean = new double[test.size()];
		for(int n =0 ;n<test.size();n++){
			for(int m=0;m<T;m++){
				sum[n] = sum[n] + result[m][n];
			}
			mean[n] = sum[n]/T;
		}
		
		System.out.println("finished calculate mean");
		double errornum=0;
		for(int aa = 0; aa<test.size(); aa++){
			errornum = errornum + (labels[aa]-mean[aa])*(labels[aa]-mean[aa]);
		}
		double rate = errornum/test.size();
		System.out.println("Final Testing mse"+" "+rate);
